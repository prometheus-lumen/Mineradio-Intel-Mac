...
function isLocalMineradioUrl(rawUrl) {
  try {
    const parsed = new URL(rawUrl);
    return (parsed.hostname === '127.0.0.1' || parsed.hostname === 'localhost')
      && (!mainServerPort || parsed.port === String(mainServerPort));
  } catch (e) {
    return false;
  }
}
...
function isMainWindowWebContents(webContents) {
  return !!(mainWindow && !mainWindow.isDestroyed() && webContents && webContents.id === mainWindow.webContents.id);
}
...
async function loadMainWindowUrl(win, url) {
  let lastError = null;
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    try {
      ...
      console.warn(`Main window load failed (${attempt}/5): ${message}`);
      if (!win || win.isDestroyed() || attempt >= 5) break;
...
function getWindowState(win) {
  const isSimpleFullScreen = !!(win && !win.isDestroyed() && process.platform === 'darwin' &&
...
function exitFullscreenToWindow(win) {
  ...
  if (!win.isFullScreen() && !isSimpleFullScreen) {
  ...
  if (isSimpleFullScreen) win.setSimpleFullScreen(false);
...
function toggleFullscreen(win) {
  if (!win || win.isDestroyed()) return;
  const isSimpleFullScreen = process.platform === 'darwin' && typeof win.isSimpleFullScreen === 'function' && win.isSimpleFullScreen();
  if (win.isFullScreen() || isSimpleFullScreen || windowFullscreenActive) {
...
ipcMain.handle('mineradio-request-camera-access', async () => {
  ...
  try {
    ...
    return { ok: !!granted, status: granted ? 'granted' : systemPreferences.getMediaAccessStatus('camera') };
  } catch (e) {
