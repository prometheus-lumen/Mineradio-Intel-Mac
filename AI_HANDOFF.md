...
- 修复普通 QQ 会话挤掉 QQ 音乐登录：此前登录窗口会混合全部 `qq.com` Cookie，关闭时 `skey/p_skey` 也会被当作成功并覆盖 `.qq-cookie`。现在主动登录先清空隔离分区，桌面端和服务端都必须取得 `qm_keyst/qqmusic_key/music_key/wxskey` 才允许保存；同时保留加密 UIN/OpenID 昵称过滤。
- 修复 Touch Bar 新增喜欢按钮后的回归：喜欢、上一首、播放/暂停、下一首四个按钮紧邻排列，歌词宽度设为 320px，仅在控制区与歌词间保留一个小间隔；心形从主进程 SVG 解码改为渲染器 Canvas PNG，空心白色/实心粉色均使用 overlay 居中。
