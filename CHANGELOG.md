...
- 修复普通 QQ 网页会话被误判为 QQ 音乐登录并覆盖原会话的问题：登录窗口使用全新隔离会话，只有取得 QQ 音乐专属播放票据后才允许保存；同时过滤加密 UIN/OpenID 昵称。
- 修复新增喜欢按钮后 Touch Bar 歌词被挤出可见区域的问题，四个控制按钮紧邻排列并将歌词画布宽度调整为 320px。
- Touch Bar 喜欢按钮改由渲染器生成 PNG 心形，确保空心白色与粉红色实心状态都能居中显示，不再依赖 macOS 下可能解码为空的 SVG。
- 修复自动发布脚本在 Node `fetch` 无法连接 GitHub API 时中断的问题，改用系统 `curl`；新增 `--resume` 断点续传并复用已生成的 DMG/补丁资产。
- 主页大卡片的时钟模块改为永久透明，外层日历和时间分块不再覆盖大卡片图片或视频背景，且不提供单独配置。
- 更新入口默认隐藏，仅检测到更高版本后显示；手动检查为最新版时只提示结果，不再打开更新面板。
- 快速补丁应用成功后自动重启 Mineradio，并增加重启防重复触发状态。
